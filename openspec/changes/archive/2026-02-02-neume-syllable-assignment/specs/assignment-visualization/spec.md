...
### Requirement: Curves connect neume center to closest syllable edge

Each curve SHALL start at the center of the neume's bounding box and end at the closest point on the syllable's bounding box edge.

#### Scenario: Neume above syllable connects to top edge
- **WHEN** a neume's center X is within the syllable's horizontal bounds
- **THEN** the curve endpoint SHALL be on the syllable's top edge at the neume's X position

#### Scenario: Neume to upper-left connects to top-left area
- **WHEN** a neume's center X is left of the syllable's left edge
- **THEN** the curve endpoint SHALL be the top-left corner of the syllable box
